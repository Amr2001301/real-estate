  ...
  return (
    <div className="space-y-5">
      ...
      <PremiumPageHero
        ...
        actions={
          <Link href={`/dashboard/brokers/${id}` as never}>
            <Button variant="outline" size="md">العودة للوسيط</Button>
          </Link>
        }
      />
      ...
      {tab === 'projects' && (
        <>
          {/* Grant form */}
          <PremiumSectionCard title="منح صلاحية وصول لمشروع" icon={<Building2 />}>
            ...
            <form action={grantProject} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <FormField label="نسبة العمولة (%)" hint="اختياري — تتجاوز النسبة الافتراضية">
                <Input id="commissionPct" name="commissionPct" type="number" min={0} max={100} step="0.01" />
              </FormField>
              <FormField label="مبلغ ثابت لكل وحدة" hint="عند نموذج FIXED_PER_UNIT">
                <Input id="fixedAmountPerUnit" name="fixedAmountPerUnit" type="number" min={0} step="0.01" />
              </FormField>
              <FormField label="بدء السريان">
                <Input id="startsAt" name="startsAt" type="date" />
              </FormField>
              <FormField label="انتهاء السريان">
                <Input id="endsAt" name="endsAt" type="date" />
              </FormField>
              {/* مفعّل + submit on the same row */}
          ...
          <PremiumSectionCard
            ...
            icon={<Building2 />}
            trailing={
              <span className="text-xs text-slate-400 tabular-nums">{access.projects.length} مشروع</span>
            }
            padded={false}
          >
            {access.projects.length === 0 ? (
              <PremiumEmptyState
                icon={<Building2 />}
                title="لا توجد مشاريع ممنوحة بعد"
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm min-w-[640px]">
                  <thead className="bg-canvas/50 border-b border-hairline">
                    <tr>
                      <th className="px-5 py-3 text-start text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400 whitespace-nowrap">المشروع</th>
                      ...
                      <th className="px-5 py-3 text-start text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400 whitespace-nowrap">العمولة</th>
                      <th className="px-5 py-3 text-start text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400 whitespace-nowrap">سريان</th>
                      <th className="px-5 py-3 text-start text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400 whitespace-nowrap">الإضافة</th>
                        ...
                        >
                          ...
                          <td className="px-5 py-3.5">
                            <span className="text-[12px] font-medium text-slate-700" dir="ltr">
                              {pct ?? fixed ?? (
                                <span className="text-slate-400 font-normal">يستخدم الافتراضي</span>
                              )}
                            </span>
                          </td>
                          <td className="px-5 py-3.5">
                          ...
                          <td className="px-5 py-3.5">
                            {pa.active && (
                              <ConfirmingForm
                                ...
                                confirmMessage={`هل تريد إلغاء صلاحية الوسيط على مشروع "${tx(pa.project.name)}"؟`}
                              >
                                <button
                                  type="submit"
      ...
      {tab === 'units' && (
        <>
          {/* Grant form */}
          <PremiumSectionCard title="منح صلاحية وصول لوحدة" icon={<Home />}>
            <p className="text-[12px] text-slate-500 mb-5">
          ...
          {/* Units list */}
          <PremiumSectionCard
            title="الوحدات المقيدة"
            icon={<Home />}
            trailing={
              <span className="text-xs text-slate-400 tabular-nums">{access.units.length} وحدة</span>
            }
            padded={false}
          >
            {access.units.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm min-w-[600px]">
                  <thead className="bg-canvas/50 border-b border-hairline">
                    <tr>
                      <th className="px-5 py-3 text-start text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400 whitespace-nowrap">الوحدة</th>
                      <th className="px-5 py-3 text-start text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400 whitespace-nowrap">المشروع</th>
                      <th className="px-5 py-3 text-start text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400 whitespace-nowrap">حالة الوحدة</th>
                      <th className="px-5 py-3 text-start text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400 whitespace-nowrap">صلاحية</th>
                      <th className="px-5 py-3 text-start text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400 whitespace-nowrap">الإضافة</th>
                      <th className="px-5 py-3 text-start w-px"></th>
                    </tr>
                  </thead>
                        ...
                        <td className="px-5 py-3.5">
                          {ua.active && (
                            <ConfirmingForm
                              ...
                              confirmMessage={`هل تريد إلغاء صلاحية الوسيط على الوحدة ${ua.unit.code}؟`}
                            >
                              <button
                                type="submit"
                                className="inline-flex items-center rounded-lg bg-danger-50 border border-danger-100 px-2.5 py-1 text-[11px] font-semibold text-danger-700 hover:bg-danger-100 transition-colors whitespace-nowrap"
