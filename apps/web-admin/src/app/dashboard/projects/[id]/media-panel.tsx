      ...
      {media.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {media.map((m: Media) => (
            <div
              key={m.id}
              className="relative group rounded-2xl overflow-hidden ring-1 ring-inset ring-hairline aspect-[4/3] bg-surface-muted"
            >
              {m.type === 'VIDEO' ? (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  start(() =>
                    deleteProjectMediaAction(project.id, m.id).then(() =>
                      router.refresh(),
                    ),
                  );
                }}
