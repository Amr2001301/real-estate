...
export default async function SalesPerformancePage({
  ...
  const m = uiT(locale).salesPerformancePage;
  ...
  const periodStr = `${m.months[parseInt(mo ?? '1', 10) - 1] ?? mo} ${y}`;
  ...
  function theme(pct: number | null): Theme {
    if (pct === null) return { bar: 'bg-slate-200', text: 'text-slate-400', accent: '#94a3b8', badge: 'bg-slate-100 text-slate-500', label: m.themeNoTarget };
    if (pct >= 100)   return { bar: 'bg-emerald-400', text: 'text-emerald-700', accent: '#34d399', badge: 'bg-emerald-100 text-emerald-700', label: m.themeExcellent };
    if (pct >= 80)    return { bar: 'bg-emerald-400', text: 'text-emerald-600', accent: '#34d399', badge: 'bg-emerald-50 text-emerald-600', label: m.themeGood };
    if (pct >= 50)    return { bar: 'bg-amber-400',   text: 'text-amber-700',   accent: '#fbbf24', badge: 'bg-amber-100 text-amber-700',   label: m.themeAverage };
    return                  { bar: 'bg-red-400',     text: 'text-red-600',     accent: '#f87171', badge: 'bg-red-100 text-red-600',     label: m.themeWeak };
  }
  ...
  const pipelineFlow = (() => {
    const stages = [
      { label: m.pipelineStageOpenLeads,        value: totalOpenLeads,  bar: 'bg-sky-400',     text: 'text-sky-700',     dot: 'bg-sky-400' },
      { label: m.pipelineStageUpcomingVisits,   value: totalUpcoming,   bar: 'bg-violet-400',  text: 'text-violet-700',  dot: 'bg-violet-400' },
      { label: m.pipelineStageActiveReservations, value: totalActiveRes, bar: 'bg-amber-400',  text: 'text-amber-700',   dot: 'bg-amber-400' },
      { label: m.pipelineStageConverted,        value: totalConvRes,    bar: 'bg-emerald-400', text: 'text-emerald-700', dot: 'bg-emerald-400' },
    ];
  ...
  return (
    <div className="space-y-5">
      ...
      <PremiumPageHero
        title={m.title}
        description={m.description(periodStr)}
        breadcrumbs={[
          { label: m.breadcrumbDashboard, href: '/dashboard' },
          { label: m.breadcrumbSales },
      ...
      <PremiumFilterBar
        ...
        trailing={
          <>
            <Button type="submit" variant="primary" size="sm">{m.filterApplyBtn}</Button>
            {hasFilter && (
              <Link href="/dashboard/sales/performance">
                <Button type="button" variant="ghost" size="sm">{m.filterClearBtn}</Button>
              </Link>
            )}
          </>
        }
      >
        <PremiumFilterField label={m.filterPeriodLabel} htmlFor="period">
          <div className="flex items-center gap-1.5">
            <CalendarDays className="h-4 w-4 text-slate-400" />
            <Input id="period" name="period" inputSize="sm" type="month" defaultValue={period} className="w-40" />
          </div>
        </PremiumFilterField>
        <PremiumFilterField label={m.filterSalesLabel} htmlFor="salesId">
          <Select id="salesId" name="salesId" inputSize="sm" defaultValue={sp.salesId ?? ''} className="w-52">
            <option value="">{m.filterAllSales}</option>
            {actors.map((a) => (
        ...
        metrics={[
          {
            ...
            sub:       rowsWithTarget.length > 0
                         ? m.kpiAvgSub(teamTheme.label, rowsWithTarget.length, rows.length)
                         : m.kpiAvgNoTargets,
            primary: true,
          },
          {
          ...
          {
            label: m.kpiContracts,
            value: String(totalContracts),
            icon:  <FileText />,
            tone:  'purple',
            sub:   m.kpiContractsSub(totalLeads),
          },
          {
            label:     m.kpiBonus,
      ...
      {rows.length === 0 ? (
        ...
        <PremiumSectionCard
          icon={<BarChart3 />}
          title={m.tableTitle}
          description={m.tableDesc(rows.length, periodStr)}
          trailing={
            <div className="hidden lg:flex items-center gap-4 text-[10px] text-slate-400">
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-sky-300 inline-block" />{m.legendLeads}</span>
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-violet-300 inline-block" />{m.legendVisits}</span>
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-amber-300 inline-block" />{m.legendReservations}</span>
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-emerald-300 inline-block" />{m.legendContracts}</span>
            </div>
          }
          padded={false}
        >
          ...
          <div className="hidden lg:grid grid-cols-[48px_1fr_130px_190px_56px_56px_56px_56px] items-center gap-2 px-5 py-3 bg-canvas/50 border-b border-hairline">
            <div />
            <p className="text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400">{m.colAgent}</p>
            <p className="text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400 text-center">{m.colAttainment}</p>
            <p className="text-[11px] font-bold uppercase tracking-[0.06em] text-slate-400">{m.colAchievedVsTarget}</p>
            <div className="flex flex-col items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-sky-400" />
              <span className="text-[9.5px] font-bold text-sky-600">{m.legendLeads}</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-violet-400" />
              <span className="text-[9.5px] font-bold text-violet-600">{m.legendVisits}</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-amber-400" />
              <span className="text-[9.5px] font-bold text-amber-600">{m.legendReservations}</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-emerald-400" />
              <span className="text-[9.5px] font-bold text-emerald-600">{m.legendContracts}</span>
            </div>
          </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <p className="text-[10px] text-slate-400">{row.achievedUnits} {m.unitsSuffix}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="hidden lg:block min-w-0 space-y-1">
                    ...
                    {row.targetUnits !== null && (
                      <p className="text-[13px] font-semibold tabular-nums text-slate-500">
                        {row.achievedUnits} / {row.targetUnits} {m.unitsSuffix}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="lg:hidden flex items-center justify-between gap-3 pt-2 mt-2 border-t border-hairline/60">
                    ...
                    <div className="flex items-center gap-3">
                      <MobilePill label={m.legendLeads.charAt(0)} value={row.leadsCount}           color="text-sky-600" />
                      <MobilePill label={m.legendVisits.charAt(0)} value={row.visitsCount}         color="text-violet-600" />
                      <MobilePill label={m.legendReservations.charAt(0)} value={row.reservationsCount} color="text-amber-600" />
                      <MobilePill label={m.legendContracts.charAt(0)} value={row.signedContractsCount} color="text-emerald-600" />
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[48px_1fr_130px_190px_56px_56px_56px_56px] items-center gap-2 px-5 py-4 bg-canvas/50 border-t-2 border-hairline/60">
            ...
            <p className="text-[11px] font-bold text-slate-500 uppercase tracking-[0.06em]">{m.totalsLabel}</p>
            ...
            <div className="px-5 py-5 space-y-0">
              ...
              {pipelineOverallConv !== null && (
                <div className="mt-4 pt-4 border-t border-hairline flex items-center justify-between gap-2">
                  <span className="text-[11px] font-medium text-slate-500">{m.pipelineOverallConvLabel}</span>
          ...
          <PremiumSectionCard
            ...
            trailing={
              <Link href="/dashboard/bonus">
                <Button variant="ghost" size="sm">{m.bonusViewAllBtn}</Button>
              </Link>
            }
            padded={false}
          >
            {bonus.length === 0 ? (
              <PremiumEmptyState
                icon={<BadgePercent />}
                title={m.bonusEmptyTitle}
                description={m.bonusEmptyDesc}
              />
            ) : (
              <>
                <div className="divide-y divide-hairline">
                  {bonus.map((entry) => {
                    const S = {
                      PENDING:  { label: m.bonusStatusPending,  cls: 'bg-slate-100 text-slate-600' },
                      APPROVED: { label: m.bonusStatusApproved, cls: 'bg-amber-100 text-amber-700' },
                      PAID:     { label: m.bonusStatusPaid,     cls: 'bg-emerald-100 text-emerald-700' },
                    } as const;
                ...
                <div className="flex items-center justify-between px-5 py-3 bg-canvas/30 border-t border-hairline">
                  <div className="flex items-center gap-3">
                    {(['PENDING', 'APPROVED', 'PAID'] as const).map((st) => {
                      ...
                      const L = { PENDING: m.bonusFooterPending, APPROVED: m.bonusFooterApproved, PAID: m.bonusFooterPaid };
