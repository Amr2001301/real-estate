...
export default async function NewUnitPage() {
  ...
  return (
    <div className="flex flex-col gap-5 lg:gap-6">
        ...
        <div className="px-7 sm:px-9 pt-8 pb-7">
          ...
          <nav aria-label="breadcrumb" className="mb-5">
            <ol className="flex flex-wrap items-center gap-1 text-xs text-slate-400">
              ...
              <li>
                <span className="font-semibold text-slate-600">وحدة جديدة</span>
              </li>
            </ol>
          </nav>
          ...
          <div className="flex items-start justify-between gap-6">
            <div className="min-w-0">
              ...
              <p className="mt-2 text-sm text-slate-500 leading-relaxed max-w-md">
                أنشئ وحدة جديدة وحدّد بياناتها الأساسية، المواصفات الفنية، والتسعير.
              </p>
            </div>
      ...
      {r.error ? (
        <div className="rounded-[18px] bg-danger-50 border border-danger-100 text-danger-700 p-4 text-sm">
          {r.error}
        </div>
      ) : (
