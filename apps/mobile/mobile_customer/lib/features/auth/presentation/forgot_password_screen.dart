  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<ForgotPasswordCubit, ForgotPasswordState>(
      listenWhen: (a, b) =>
          ...
          (a.step != b.step && b.step == ForgotResetStep.done),
      listener: (context, state) {
        if (state.step == ForgotResetStep.done) {
      ...
      builder: (context, state) {
        if (state.step == ForgotResetStep.resetForm) {
          return _ResetForm(
            l10n: l10n,
            ...
            onSubmit: () {
              if (_resetFormKey.currentState?.validate() ?? false) {
                context.read<ForgotPasswordCubit>().submitReset(
        ...
        return _RequestForm(
          ...
          onSubmit: () {
            if (_emailFormKey.currentState?.validate() ?? false) {
              context.read<ForgotPasswordCubit>().sendResetLink(_emailCtrl.text.trim());
            }
          },
        );
      },
